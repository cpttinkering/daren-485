...
# See https://github.com/cpttinkering/daren-485 for a protocol research information
    ...
    def get_cap_params(self, ser):
        ...
        if response != False:
            ...
            if len(payload) >= 36: # 9*4 bytes in full request.
                ...
                self.history.total_ah_drawn = int(payload[20:28], base=16) #total_discharge_capacity
    ...
    def get_realtime_data(self, ser):
        ...
        if response != False:
            ...
            if len(payload) >= 118:
                ...
                if voltagestatus & (1 << 2) or voltagestatus & (1 << 0): #check bit 2 for TOT_OVV_PROT and bit 0 for cell_OVV_PROT
                    self.protection.high_voltage = 2
                elif voltagestatus & (1 << 6) or voltagestatus & (1 << 4): #check bit 6 for TOT_OVV_alarm and 4 for cell_OVV_alarm
                ...
                if voltagestatus & (1 << 3): #check bit 3 for TOT_UNDV_PROT
                    self.protection.low_voltage = 2
                elif voltagestatus & (1 << 7):  #check bit 7 for TOT_UNDV_alarm
                    self.protection.low_voltage = 1
                else:
                    self.protection.low_voltage = 0

                if voltagestatus & (1 << 1): #check bit 1 for cell_UNDV_PROT
                    self.protection.low_cell_voltage = 2
                elif voltagestatus & (1 << 5):  #check bit 5 for cell_UNDV_alarm
                    self.protection.low_cell_voltage = 1
                else:
                    self.protection.low_cell_voltage = 0

                if currentstatus & (1 << 2): #check bit 2 for CHG_OC_PROT 
                    self.protection.high_charge_current = 2
                elif currentstatus & (1 << 6): #check bit 6 for CHG_C_alarm
                    self.protection.high_charge_current = 1
                else: 
                    self.protection.high_charge_current = 0 #high charge current

                if currentstatus & (1 << 4) or currentstatus & (1 << 5) or currentstatus & (1 << 3): #check bit 4 for DISCH_OC_1_PROT, bit 5 for DISCH_OC_2_PROT and bit 3 for Short_circuit_PROT
                    self.protection.high_discharge_current = 2
                elif currentstatus & (1 << 7): #check bit 7 for DISCH_C_alarm
                    self.protection.high_discharge_current = 1
                else: 
                    self.protection.high_discharge_current = 0 #high charge current

                if voltagestatus & (1 << 14): #check bit 14 for V_DIF_PROT
                    self.protection.cell_imbalance = 2
                elif voltagestatus & (1 << 8): #check bit 8 for V_DIF_ALARM
                    self.protection.cell_imbalance = 1
                else:
                    self.protection.cell_imbalance = 0
                
                if warningstatus > 0: #if something is in warning, report internal failure. warningstatus contains all sorts of internal components, such as CHG_FET, NTC_fail, cell_fail, chg_mos_fail, disch_mos_fail, etc.
                    self.protection.internal_failure = 2
                else:
                    self.protection.internal_failure = 0
                
                if tempstatus & (1 << 0): #check bit 0 for CHG_H_TEMP_PROT
                    self.protection.high_charge_temp = 2
                elif tempstatus & (1 << 8): #check bit 8 for CHG_H_TEMP_alarm
                    self.protection.high_charge_temp = 1
                else: 
                    self.protection.high_charge_temp = 0

                if tempstatus & (1 << 1): #check bit 1 for CHG_L_TEMP_PROT
                    self.protection.low_charge_temp = 2
                elif tempstatus & (1 << 9): #check bit 9 for CHG_L_TEMP_alarm
                    self.protection.low_charge_temp = 1
                else: 
                    self.protection.low_charge_temp = 0

                if tempstatus & (1 << 0) or tempstatus & (1 << 2): #check bit 0 for CHG_H_TEMP_PROT and bit 2 for DISCH_H_TEMP_PROT
                    self.protection.high_temperature = 2
                elif tempstatus & (1 << 8) or tempstatus & (1 << 10): #check bit 8 for CHG_H_TEMP_alarm and bit 10 for DISCH_H_TEMP_alarm
                    self.protection.high_temperature = 1
                else: 
                    self.protection.high_temperature = 0

                if tempstatus & (1 << 1) or tempstatus & (1 << 3): #check bit 1 for CHG_L_TEMP_PROT and bit 3 for DISCH_L_TEMP_PROT
                    self.protection.low_temperature = 2
                elif tempstatus & (1 << 9) or tempstatus & (1 << 11): #check bit 9 for CHG_L_TEMP_alarm and bit 11 for DISCH_L_TEMP_alarm
                    self.protection.low_temperature = 1
                else: 
                    self.protection.low_temperature = 0

                if tempstatus & (1 << 6) or tempstatus & (1 << 4): #check bit 6 for MOS_H_TEMP_PROT and 4 for ENV_H_TEMP_PROT
                    self.protection.high_internal_temp = 2
                elif tempstatus & (1 << 14) or tempstatus & (1 << 12): #check bit 14 for MOS_H_TEMP_alarm and 12 for ENV_H_TEMP_alarm
